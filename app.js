...
// Add this to the very beginning of app.js (before the TripMaster class definition)

// Global error handler to catch circular reference errors
window.addEventListener('error', (event) => {
    if (event.message && event.message.includes('circular')) {
        ...
        console.log('- Current tab:', window.tripMaster?.state?.activeTab);
        console.log('- Trip location:', window.tripMaster?.state?.trip?.location);
        console.log('- User profile:', window.tripMaster?.userProfile?.name);
        console.log('- Loading state:', window.tripMaster?.state?.isLoading);
...
// Also catch unhandled promise rejections with circular reference
window.addEventListener('unhandledrejection', (event) => {
    if (event.reason && event.reason.message && event.reason.message.includes('circular')) {
        console.error('🚨 CIRCULAR REFERENCE IN PROMISE:');
        console.error('Reason:', event.reason.message);
        console.error('Promise stack:', event.reason.stack);
        
        console.log('🔍 Promise context when error occurred:');
        console.log('- Current tab:', window.tripMaster?.state?.activeTab);
        console.log('- Trip location:', window.tripMaster?.state?.trip?.location);
        console.log('- User profile:', window.tripMaster?.userProfile?.name);
        console.log('- Loading state:', window.tripMaster?.state?.isLoading);
...
// Override JSON.stringify to catch where circular references are being created
...
JSON.stringify = function(value, replacer, space) {
    try {
        ...
        if (error.message.includes('circular')) {
            console.error('🚨 JSON.stringify CIRCULAR REFERENCE:');
            console.error('Error:', error.message);
            console.error('Stack trace:', error.stack);
            
            // Try to identify what object was being stringified
            console.log('🔍 Attempted to stringify:', typeof value);
            if (typeof value === 'object' && value !== null) {
                console.log('🔍 Object keys:', Object.keys(value));
                
                // Check each property individually to find the circular one
                for (const [key, val] of Object.entries(value)) {
                    try {
                        originalStringify.call(this, val);
                        console.log(`✅ Property "${key}" is OK`);
                    } catch (propError) {
                        if (propError.message.includes('circular')) {
                            console.error(`❌ Property "${key}" has circular reference`);
                            console.log(`🔍 Property "${key}" type:`, typeof val);
                            if (typeof val === 'object' && val !== null) {
                                console.log(`🔍 Property "${key}" keys:`, Object.keys(val));
                            }
                        }
                    }
                }
            ...
            // Get stack trace to see what code called JSON.stringify
            const stack = new Error().stack;
            console.log('🔍 JSON.stringify called from:', stack);
        }
